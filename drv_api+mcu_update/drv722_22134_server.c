...
pthread_mutex_t mcu_update_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void* api_answer_thread(void *arg)
{
	...
	pthread_mutex_lock(&mcu_update_mutex);

	answer_to_api(pmsgbuf);    //处理接收到的信息

	pthread_mutex_unlock(&mcu_update_mutex);
	//内存记得释放
	free(pmsgbuf);
	return NULL;	
}
...
//多线程查询消息队列
void* mcu_update_thread(void *arg)
{
	...
	while(1)
	{		
		ret = mcuupdate_msgq_recv(TYPE_RECVFROM_API,&msgbuf,0);  //阻塞试接收，消息类型都是同一种
		if(ret == 0)   //收到信息，打印出来
		{
			if(server_in_debug_mode)			
					printf("ServerDEBUG:mcu_update_thread type = %ld cmd = %d buf=%s\n",msgbuf.types,msgbuf.cmd,msgbuf.buf);
			if(msgbuf.cmd == eMCUUPDATE_FILENAME) //是文件名，这个要绝对路径
			{
				//1.停止其他程序使用串口
				pthread_mutex_lock(&mcu_update_mutex);
				pthread_cancel(mcu_uart_rcv_thread);

				msgbuf.types = TYPE_SENDTO_API;
				msgbuf.cmd = eMCUUPDATE_STATUS;
				//2.开始升级
				if(0 == xymodem_send((const unsigned char *)msgbuf.buf))
				{
					printf("xymodem_send is success done!\n");
				}
				else
				{
					printf("xymodem_send is failed!\n");
				}
				mcuupdate_msgq_send_ack(&msgbuf);
		...
		{
			printf("Server ERROR: mcuupdate_msgq_recv\n");
			if(errno != EINTR)  //捕获到信号
			{
				printf("Server error errno != EINTR!\n");
			}			
...
static int find_es8388(void)
{
	...
	if(i2c_adapter_fd >= 0)
	{
		if(s_read_reg(i2c_adapter_fd, 0, &val)) {
...
//	int i2c_device_es8388_addr = -1;  //8388 iic设备地址
	...
	es8388i2c_adapter_fd = find_es8388();
	...
	//用于通信的消息队列
	if(0 != mcuupdate_msgq_init())
	{
		printf("error : mcuupdate_msgq_init\n");
		return -1;
	}

	pthread_create(&mcuupdate_thread, NULL, mcu_update_thread, NULL);
	pthread_create(&mcu_uart_rcv_thread, NULL, mcu_recvSerial_thread, NULL);
