/*
* @Author: dazhi
* @Date:   2023-04-24 16:08:47
* @Last Modified by:   dazhi
* @Last Modified time: 2023-04-24 16:18:01
*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define eAPI_CONTROL_TTYS0_CMD 24
...
//	int param = 0;   //参数需要一个指针，返回函数返回值
	if(Jc_msgq_init())  //未初始化
		return -1;

	if(api_send_and_waitack(eAPI_CONTROL_TTYS0_CMD,val,&val))  //发送的第二个参数表示led号，第三个表示点亮还是熄灭
	{
		printf("error : drvControlttyS0 \n");
		return -1;
	}
	printf("drvControlttyS0 ret = %d\n",val);
	return val;   //返回值！！	
