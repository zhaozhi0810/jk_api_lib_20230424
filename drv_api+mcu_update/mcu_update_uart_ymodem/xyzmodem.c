...
// {
// 	return snprintf((char*)str,arrlen-1,"%d",intnum);
...
//	printf("i+PACKET_HEADER = %d\n",i + PACKET_HEADER);
	...
	snprintf((char*)file_ptr,sizeof file_ptr-1,"%d",*length);
...
int get_file_md5sum(const unsigned char * filename)
{
	FILE * filep;
	char cmd[128] = {"md5sum "};
...
*********************************************************************************************************
*/
int xymodem_send(const unsigned char *filename)
{
//    size_t len;
    int ret;// fd;
    FILE *fin;// *fout;
 //   int skip_payload = 0;
	...
	do
	{
		ret = UART_ReceivePacket (data,1, 10);  //
	}while(ret == 0);

	if(1)  //没有收到数据，或者收到的不是0x43
	{
