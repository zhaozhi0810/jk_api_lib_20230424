...
2023-04-24
1.捕获中止信号，捕获信号的时候，发出中止看门狗的指令
2.增加临时关闭串口的接口。用于单片机串口升级操作。
3.重新修改查找8388地址的函数。




2023-06-15  主要是libdrv722_22134.so
由于声音通道关闭的时候需要保存现有音量值，在连续关闭两次的时候，保存的音量值为0，
导致再要开启的时候，无法再开启（扬声器通道），此时音量值为0。
修复该问题：对通道的状态进行判断，如果已经被关闭了，则不再进行关闭操作。
本次需要更新libdrv722_22134.so 到/usr/lib目录中

hj的音量这里需要drvso来自己保存，目前已经做了全局变了保存了。



2023-10-20  主要是libdrv722_22134.so
1.因测试需要，关闭了强声器的断开功能（继电器不能松开），drvDisableWarning函数直接返回
2.1中的修改已经还原
